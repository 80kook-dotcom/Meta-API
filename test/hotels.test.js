...
test('호텔 1건 매핑: 한글명·평점÷2·propertyType 패싯·이미지·amenities', async () => {
  ...
  assert.equal(h.numberOfProviders, 9)
...
test('isDirect 공급사 요금 → cashback NONE (#5)', async () => {
  const { results } = await adaptHotels(RAW)
  const directRate = results[0].topRates.find((r) => r.providerName === '사이트 직접 예약')
  assert.deepEqual(directRate.cashback, { type: 'NONE' })
})
