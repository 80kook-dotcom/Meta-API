...
test('providers: 「사이트 직접 예약」(isDirect) 제외 · 남은 공급사는 원본 index 보존(조인키)', () => {
  const d = adaptHotelDetail(RAW)
  // RAW providers = [사이트직접(index0·direct), 아고다(index1)] → direct 제외 → 아고다만 남음.
  assert.equal(d.providers.length, 1)
  // 🔑 아고다의 index 가 원본 위치(1)를 그대로 유지해야 results.providerIndex(1)와 조인된다(재매핑 없음).
  assert.deepEqual(d.providers[0], { index: 1, name: '아고다', logo: '아', cashback: { type: 'PERCENTAGE', value: 5.5, cap: 1200000, currency: 'KRW' }, logoUrl: 'https://cdn/agoda.png' })
})

test('results: direct 요금 제외 · 남은 요금의 providerIndex 가 원본 위치 유지(조인 보존)', () => {
  const d = adaptHotelDetail(RAW)
  // 스탠다드(providerIndex0=사이트직접·direct) 제외 → 디럭스(providerIndex1=아고다)만 남음.
  assert.equal(d.results.length, 1)
  const deluxe = d.results[0]
  assert.equal(deluxe.roomName, '디럭스')
  assert.equal(deluxe.providerIndex, 1) // 🔑 원본 위치 유지 → providers[index=1]=아고다와 조인
...
test('results.isCheapestRate: direct 제외 후 남은 요금 중 최저가만 true', () => {
  const d = adaptHotelDetail(RAW)
  // 스탠다드(direct) 제외 → 디럭스(원본 false)만 남고 단독이므로 최저가 = true 로 교정.
  assert.equal(d.results.length, 1)
  assert.equal(d.results[0].roomName, '디럭스')
  assert.equal(d.results[0].isCheapestRate, true)
})
...
test('dataless 방어: results 없음 → results []·providers 매핑(direct 제외)·facilities 유지', () => {
  const raw = { ...RAW, results: undefined }
  const d = adaptHotelDetail(raw)
  assert.deepEqual(d.results, [])
  assert.equal(d.providers.length, 1) // 아고다만(「사이트 직접 예약」 direct 제외)
...
test('적대리뷰 #2/#4/#5: 무효 요금(totalRate<=0·providerIndex<0) 드롭 + isCheapestRate 정합', () => {
  const raw = {
    ...RAW,
    results: [
      { roomName: '정상', totalRate: 300000, providerIndex: 1 }, // 아고다(non-direct)
      ...
      { roomName: '정상2', totalRate: 250000, providerIndex: 1 }, // 아고다(무효요금 드롭만 검증·direct 제외와 분리)
    ],
  }
  const d = adaptHotelDetail(raw)
...
test('direct 가 유일 공급사: providers·results 모두 빔(딥링크 직접진입 시 가격비교 빈 상태)', () => {
  const raw = {
    ...RAW,
    providers: [{ code: 'X', name: '사이트 직접 예약', isDirect: true }],
    results: [{ roomName: 'x', totalRate: 100000, providerIndex: 0 }],
  }
  const d = adaptHotelDetail(raw)
  assert.deepEqual(d.providers, []) // 유일 공급사가 direct → 전부 제외
  assert.deepEqual(d.results, []) // 그 요금도 제외
})
